...
void evolve(double *psi_new, double *psi_old, double delta_t, double delta_x,
            int n_x);
...
void evolve(double *psi_new, double *psi_old, double delta_t, double delta_x,
  ...
  for (i = 1; i < n_x - 1; i++) {
    psi_new[i] = psi_old[i];
    psi_new[i] += (delta_t / (delta_x * delta_x)) *
                  (psi_old[i + 1] - 2 * psi_old[i] + psi_old[i - 1]);
    psi_new[i] += delta_t * s;
  }
}
